...
class Matrix {
  ...
  static mul(A, B) {
    ...
    for (var i = 0; i < m; i++)
      for (var j = 0; j < p; j++)
        for (var k = 0; k < n; k++) C[i][j] += A[i][k] * B[k][j];
    return C;
  }
...
class Camera {
  ...
  update() {
    ...
    this.R = [
      [
        cosθ * cosψ,
        cosθ * sinψ * sinφ - sinθ * cosφ,
        cosθ * sinψ * cosφ + sinθ * sinφ,
      ],
      [
        sinθ * cosψ,
        sinθ * sinψ * sinφ + cosθ * cosφ,
        sinθ * sinψ * cosφ - cosθ * sinφ,
      ],
...
class TriHex extends Hex {
  ...
  unit() {
    ...
    return [
      tri,
      [1, 2, 3, 4, 5].map((e) => tri.clone().rotate(60 * e, [0, 0])),
      hex,
    ].flat();
  }
}
...
class RhombiTriHex extends Hex {
  ...
  unit() {
    var hex = new Path.RegularPolygon([0, 0], 6, this.R);
    hex.name = "mer-1";
    var sqr = new Path.RegularPolygon(
      [0, 0],
      4,
      Math.sqrt(2 * this.R * this.R) / 2
    );
    sqr.name = "mer-3";
    ...
    return [
      sqr,
      [1, 2, 3, 4, 5].map((e) => sqr.clone().rotate(60 * e, [0, 0])),
      tri,
      [1, 2, 3, 4, 5].map((e) => tri.clone().rotate(60 * e, [0, 0])),
      hex,
    ].flat();
...
class DualSnubHex extends Hex {
  ...
  unit() {
    ...
    return [
      path,
      [1, 2, 3, 4, 5].map((e) => path.clone().rotate(e * 60, [0, 0])),
    ].flat();
  }
}
...
function drawIco(face, R, F, P, opt) {
  ...
  const A = Matrix.inv3([
    [
      face.bounds.topCenter.x,
      face.bounds.bottomLeft.x,
      face.bounds.bottomRight.x,
    ],
    [
      face.bounds.topCenter.y,
      face.bounds.bottomLeft.y,
      face.bounds.bottomRight.y,
    ],
    [1, 1, 1],
  ]);
  ...
  var fibers = [];
  if (F > 0) {
    var p1 = RegularIcosahedron.verts(R, P).map((e) => [
      e[0][0],
      e[1][0],
      e[2][0],
    ]);
    var p2 = RegularIcosahedron.verts(F, P).map((e) => [
      e[0][0],
      e[1][0],
      e[2][0],
    ]);
